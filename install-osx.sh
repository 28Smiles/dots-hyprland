...
sudo cp firmware/OVMF_VARS-1024x768.fd /var/lib/libvirt/qemu/nvram/macOS_VARS-1280x720.fd
...
sed -e "s|VMDIR/firmware/OVMF_CODE.fd|/var/lib/libvirt/qemu/nvram/macOS_CODE.fd|g" \
    -e "s|VMDIR/firmware/OVMF_VARS-1024x768.fd|/var/lib/libvirt/qemu/nvram/macOS_VARS-1280x720.fd|g" \
    ...
    -e "s|<currentMemory unit='KiB'>2097152</currentMemory>|<currentMemory unit='KiB'>$MEMORY</currentMemory>|g" \
    -e "s|<vcpu placement='static'>4</vcpu>|<vcpu placement='static'>$CORES</vcpu>|g" \
    -e "s|<topology sockets='1' cores='4' threads='1'/>|<topology sockets='1' cores='$CORES' threads='1'/>|g" \
